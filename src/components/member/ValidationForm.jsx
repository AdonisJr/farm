...
export default function ValidationForm({ user, accessToken, setShowForm, getData }) {
    const [credentials, setCredentials] = useState(
        {
            user_id: user.id, birth_date: user.birth_date, phone_number: user.phone_number,
            barangay: user.barangay, first_name: user.first_name, middle_name: user.middle_name, last_name: user.last_name, status: "PENDING"
        });
    ...
    const handleSubmit = async () => {
        ...
        await axios.post(`/validation`, filesData, {
            headers: {
                ...
                <div className='relative bg-white flex flex-col gap-3 w-4/6 p-5 text-sm'>
                    ...
                    <div className='flex gap-2'>
                        <div className="flex flex-col w-full">
                            <label className="ps-2">Valid ID</label>
                            <input
                                type="file"
                                placeholder="Taylor"
