...
export default function AdmRice({ user, accessToken, farms }) {
    ...
    const preparePrintData = () => {
        ...
        printData += '</style>';
        printData += `<h1 class="title">RCEF RICE SEED DISTIBUTION Subsidy Report</h1>`;
        printData += '<table>';
        printData += '<thead>';
        printData += '<tr>';
    ...
    const getSubsidies = async () => {
        await axios
            .get(`/subsidy?type=RCEF RICE SEED DISTIBUTION&q=${q}&barangay=${barangay}&month=${month}&year=${year}`, {
                headers: {
