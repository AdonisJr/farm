...
export default function AdmCash({ user, accessToken, farms }) {
    ...
    const preparePrintData = () => {
        ...
        printData += '</style>';
        printData += `<h1 class="title">Cash Subsidy Report</h1>`;
        printData += '<table>';
        printData += '<thead>';
        printData += '<tr>';
    ...
    const getSubsidies = async () => {
        await axios
            .get(`/subsidy?type=CASH&q=${q}&barangay=${barangay}&month=${month}&year=${year}`, {
                headers: {
                        ...
                        !subsidies ? "" :
                            subsidies.map(subsidy => (
                                <tr className='text-center border-b-2 border-slate-200 hover:bg-slate-100 duration-200'>
                                    ...
                                    <td>{convertToPHP(subsidy.amount)}</td>
