...
export default function AdmLiquid({ user, accessToken, farms }) {
    ...
    const preparePrintData = () => {
        ...
        printData += '</style>';
        printData += `<h1 class="title">LIQUID ZINC Subsidy Report</h1>`;
        printData += '<table>';
        printData += '<thead>';
        printData += '<tr>';
    ...
    const getSubsidies = async () => {
        await axios
            .get(`/subsidy?type=LIQUID ZINC&q=${q}&barangay=${barangay}&month=${month}&year=${year}`, {
                headers: {
    ...
    return (
        <div className='p-4 bg-white rounded-lg shadow-sm text-sm'>
            ...
            <div className='flex justify-between pe-10'>
                ...
                <p className='font-bold text-lg text-slate-600 py-3'>LIQUID ZINC SUBSIDY</p>
                <button
