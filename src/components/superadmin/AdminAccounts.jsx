...
export default function AdminAccounts({ user, accessToken }) {
  ...
  const getAdmins = async () => {
    await axios
      .get(`/user/admins?id=${user.id}&q=${q}`, {
        headers: {
  ...
  return (
    <div className='flex flex-col gap-5 w-full bg-white p-2 text-sm'>
      <div>
        {
          !isModalOpen ? <></> :
            <AdminForm setModal={setModal} getAdmins={getAdmins} setSelected={setSelected} selected={selected} accessToken={accessToken} />
        }
      </div>
      <div className='w-full p-2 bg-'>
        ...
        <AdminTable admins={admins} setModal={setModal} setSelected={setSelected} selected={selected} accessToken={accessToken} />
      </div>
